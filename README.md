...
#### Дубликаты тестов и непокрытые ФТ

По результатам сопоставления выполняются две проверки:

- `assertion-duplicate-test` — с одним ФТ сопоставлено два или более теста. Учитываются тесты из всех подключенных источников, поэтому jest-тест и история storybook с одинаковым идентификатором — тоже дубликат. Запуски одного testplane-теста в разных браузерах считаются одним тестом. По умолчанию — предупреждение.
- `assertion-not-covered` — с ФТ не сопоставлен ни один тест. По умолчанию проверка выключена. Если ее включить, не подключив ни одного источника тестов, в отчет попадут все ФТ проекта.

Уровень каждой проверки задается в секции `validation` [конфигурационного файла](#формат-конфига).
...
```js
{
    ...
    "validation": {
        ...
        // с одним ФТ сопоставлено несколько тестов
        "assertion-duplicate-test": "warning",

        // с ФТ не сопоставлен ни один тест
